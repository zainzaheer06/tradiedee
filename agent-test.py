...
from livekit.agents.beta.workflows.dtmf_inputs import GetDtmfTask, DtmfEvent
...
def get_default_config():
    ...
    return {
        ...
        # Role & Objective
**Role & Identity:**
You are the AI voice assistant for TAMGO (The Machinery Group LLC), Saudi Arabia's leading provider of industrial machinery solutions. You represent a company with over 30 years of excellence, serving major industries across the Kingdom and region. Speak with professionalism, expertise, and Saudi business cultural awareness.

- Always respond in 1-2 sentences
- Do not use more than 2 sentences in response.
- Keep responses short.


**Company Overview:**
- Established 1991 as part of Zahid Group
- Headquartered in Jeddah with branches across Saudi Arabia
- ISO 9001 and ISO 45001 certified
- Official partnerships with world-leading brands (FG Wilson, Bobcat, Kubota, Ingersoll Rand, SEM, EP, AFC Energy, Peerless Pump)

**Seven Core Solutions:**
1. **Power Solutions** - Generators and power systems
2. **Construction Machinery** - Heavy equipment for construction projects
3. **Compressed Air Solutions** - Industrial air compressors
4. **Portable Power Solutions** - Mobile power units
5. **Fire & Water Solutions** - Pumps and firefighting equipment
6. **Material Handling** - Forklifts and warehouse equipment
7. **Mining Solutions** - Specialized mining machinery

**Key Industries Served:**
Mining, construction, manufacturing, oil & gas, shipping, transportation, public, and private sectors

**Your Capabilities:**
- Answer inquiries about products, services, and solutions
- Provide information on parts availability and services
- Direct inquiries to appropriate departments (Sales, Parts, Service)
- Share company locations and contact information
- Explain certifications and company values

**Conversation Style:**
- Professional yet approachable
- Use Saudi business etiquette (respect, patience, hospitality)
- Be concise but thorough
- Use Arabic greetings when appropriate: "مرحباً" (Marhaban) for hello
- Speak clearly at moderate pace for technical topics

**Contact Information:**
- Head Office: Jeddah
- Additional branches strategically positioned across KSA
- Careers email: careers@tamgo.com
- Website: tamgoksa.com

**Escalation Protocol:**
When customers need:
- Your query has been noted. Relevent person will contact you.

**Key Talking Points:**
- "Over 30 years of excellence supporting Saudi Arabia's infrastructure"
- "World-class brands with exceptional product support"
- "ISO certified for quality and safety standards"
- "Strategic locations across the Kingdom for efficient service"
- "Committed to Vision 2030 and developing Saudi talent"

**Conversation Flow:**
1. **Greeting:** Warmly welcome caller, introduce yourself as Sara from Tamgo
2. **Identify Need:** "How may I assist you today?" - listen for their solution category
3. **Provide Information:** Share relevant details about products, services, or processes
4. **Offer Next Steps:** Guide to schedule callback.
5. **Professional Closing:** Thank them, confirm any actions, professional goodbye

**Important Notes:**
- If asked about specific pricing, always connect to sales team
- For urgent equipment breakdowns, prioritize immediate transfer to service
- Mention ISO certifications when discussing quality/safety
- Highlight being part of Zahid Group for enterprise clients
- Stay within scope - don't make commitments about delivery times or warranties without verification
- if customer wants to buy or need more informations then tell them customer sales will contact you.


**Sample Responses:**

*General Inquiry:*
"مرحباً, welcome to Tamgo, The Machinery Group. I'm here to help you with industrial machinery solutions. We offer seven comprehensive solutions including power systems, construction machinery, and material handling. Which area interests you today?"

*Parts Request:*
"I can certainly help you with parts. Tamgo maintains extensive inventory for all our brands including Bobcat, Kubota, and FG Wilson. Let me connect you with our Parts Department who can check availability and provide immediate assistance."

*Service Emergency:*
"I understand this is urgent. Our service teams across Saudi Arabia are equipped to respond quickly. I'm transferring you immediately to our Service Department. Please stay on the line."
        """,
...
def build_prompt_with_raw_webhook_data(user_prompt: str, raw_data: dict) -> str:
    """
    Inject webhook data into prompt using compact format (low latency)
    Args:
        user_prompt: The prompt from database (agent.prompt)
    ...
    # Add compact webhook data section if available
    if raw_data:
        # Ultra-compact format: single line, comma-separated (minimal tokens)
        compact = ", ".join([f"{k}:{v}" for k, v in raw_data.items()])
        webhook_section = f"\n# بيانات العميل: {compact}\n"

        full_prompt = base_prompt + webhook_section
        logger.debug(f"📊 Webhook data injected ({len(raw_data)} fields, ~{len(compact)} chars)")
        return full_prompt
    else:
...
class Assistant(Agent):
    ...
    '''   
    @function_tool
    async def collect_phone_number_dtmf(self, ctx: RunContext):
        """
        Collect phone number using DTMF (keypad) - most reliable method
        Returns the phone number or None if failed
        """
        logger.info("📞 Starting DTMF phone number collection")

        try:
            # Run DTMF task - it will instruct user automatically
            result = await GetDtmfTask(
                num_digits=10,                        # Expect 10 digits (Saudi format without country code)
                ask_for_confirmation=True,            # Task will confirm automatically
                dtmf_input_timeout=15.0,               # 5 seconds between each digit (stricter)
                dtmf_stop_event=DtmfEvent.POUND,      # Press # to submit early
                extra_instructions=(
                    "CRITICAL RULES - FOLLOW EXACTLY:\n"
                    "1. You are collecting phone numbers ONLY from DTMF keypad tones (tagged as <dtmf_inputs>).\n"
                    "2. COMPLETELY IGNORE any spoken numbers or words from the user's voice.\n"
                    "3. ONLY use digits that arrive as DTMF events (shown in <dtmf_inputs> tags).\n"
                    "4. First, tell user in Arabic: 'من فضلك أدخل رقم جوالك باستخدام أزرار الهاتف، واضغط # عند الانتهاء'\n"
                    "5. Wait silently for DTMF digits. Do NOT respond to spoken words.\n"
                    "6. When you receive 10 DTMF digits OR user presses #, read back ONLY the DTMF digits in Arabic.\n"
                    "7. If user confirms (نعم، صح، تمام), call confirm_inputs with the DTMF digits.\n"
                    "8. If user rejects, tell them to re-enter using keypad buttons only."
                )
            )
            
            # ⭐ STEP 3: Check result
            if result and result.user_input:
                phone_number = result.user_input
                logger.info(f"✅ Phone number collected: {phone_number}")
                
                # Task already confirmed, so return
                return phone_number
            else:
                logger.warning("⏰ DTMF collection timeout or cancelled")
                await ctx.session.say("ما استلمت الرقم. ممكن نحاول مرة ثانية؟")
                return None
                
        except Exception as e:
            logger.error(f"❌ DTMF collection error: {e}")
            import traceback
            logger.error(traceback.format_exc())
            
            # Fallback to voice
            logger.info("🎤 Falling back to voice collection")
            return await self.collect_phone_number_voice(ctx)

    async def collect_phone_number_voice(self, ctx: RunContext):
        """Fallback: Pure voice collection"""
        logger.info("🎤 Starting voice-only phone collection")
        
        await ctx.session.say(
            "طيب، قول رقم جوالك بصوتك، رقم رقم ببطء",
            allow_interruptions=False
        )
        
        # Let the LLM handle voice collection naturally
        # This returns to normal conversation mode where LLM will extract the number
        return None

    def format_arabic_number(self, number: str) -> str:
        """Convert digits to spoken Arabic with pauses"""
        digit_map = {
            '0': 'صفر', '1': 'واحد', '2': 'اثنين', '3': 'ثلاثة',
    ...
    async def collect_phone_number_dtmf(self, ctx: RunContext):
        """
        Collect phone number using manual DTMF listener
        Most reliable method - directly listens to keypad tones
        """
        logger.info("📞 Starting manual DTMF phone collection")
        ...
        # Get job context for room access
        ...
        def on_dtmf_received(dtmf: rtc.SipDTMF):
            nonlocal digits, collection_complete
            
            if collection_complete:
                return
            
            logger.info(f"📟 DTMF: {dtmf.digit}")
            
            if dtmf.digit == '#':
                # Submit
                collection_complete = True
                logger.info(f"✅ Submitted: {''.join(digits)}")
            elif dtmf.digit == '*':
                # Clear
                digits = []
                logger.info("🔄 Cleared")
            elif dtmf.digit.isdigit():
                # Add digit
                digits.append(dtmf.digit)
                logger.info(f"📱 Digits: {''.join(digits)}")
        
        try:
            # Register listener
            job_ctx.room.on("sip_dtmf_received", on_dtmf_received)
            
            # Tell user
            await ctx.session.say(
                "ممكن تدخل رقم جوالك بأزرار الجوال؟ اضغط على الأرقام، واضغط شباك هاش بعد الانتهاء",
                allow_interruptions=False
            )
            
            # Wait for completion
            start_time = time.time()
            while not collection_complete and (time.time() - start_time) < timeout_seconds:
                await asyncio.sleep(0.1)
            
            # Check result
            if collection_complete and len(digits) >= 9:
                phone_number = ''.join(digits)
                
                # Confirm
                spoken = self.format_arabic_number(phone_number)
                await ctx.session.say(
                    f"استلمت الرقم: {spoken}. صح؟",
                ...
                logger.info(f"✅ Phone confirmed: {phone_number}")
                return phone_number
            else:
                logger.warning(f"⏰ Timeout or invalid: {len(digits)} digits")
                await ctx.session.say("ما استلمت رقم كامل")
                return None
            ...
            # Remove listener
            try:
                job_ctx.room.off("sip_dtmf_received", on_dtmf_received)
            except:
                pass

    def format_arabic_number(self, number: str) -> str:
        """Convert digits to spoken Arabic"""
        digit_map = {
            '0': 'صفر', '1': 'واحد', '2': 'اثنين', '3': 'ثلاثة',
            '4': 'أربعة', '5': 'خمسة', '6': 'ستة', '7': 'سبعة',
            '8': 'ثمانية', '9': 'تسعة'
        ...
        try:
            if participant.metadata:
                ...
                if metadata.get('type') == 'campaign':
                    ...
                    phone_number = metadata.get('phone_number')  # ⭐ Extract phone number

                    # ⭐ INJECT PHONE NUMBER into webhook_context (if no pre-call webhook)
                    if phone_number and not webhook_context:
                        webhook_context = {
                            'phone_number': phone_number,
                            'source': 'metadata_fallback'
                        }
                        logger.info(f"✅ Phone number injected into context (no pre-call webhook): {phone_number}")
                    elif phone_number and webhook_context and 'phone_number' not in webhook_context:
                        # Add phone number to existing webhook context if missing
                        webhook_context['phone_number'] = phone_number
                        logger.info(f"✅ Phone number added to existing webhook context: {phone_number}")
            ...
            if participant.metadata:
                try:
                    ...
                    if metadata.get('type') == 'outbound_api':
                        ...
                        webhook_context = metadata.get('webhook_context')
                        phone_number = metadata.get('phone_number')  # ⭐ Extracted
