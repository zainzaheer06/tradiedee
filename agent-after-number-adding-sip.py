...
You can speak in english too as well.

# CRITICAL RULE - NO JSON EVER
- You are a VOICE assistant for phone calls
- ALWAYS respond in natural spoken english/Arabic ONLY
...
- Always respond in english/Arabic only.
...
async def entrypoint(ctx: JobContext):
    ...
    # ===== GET PHONE FROM ROOM METADATA (DISPATCH METADATA) =====
    phone_from_metadata = None
    webhook_context = None

    if ctx.job.metadata:
        try:
            metadata = json.loads(ctx.job.metadata)
            phone_from_metadata = metadata.get('phone_number')
            webhook_context = metadata.get('webhook_context')
            agent_id_from_meta = metadata.get('agent_id')
            logger.info(f"📦 Dispatch metadata parsed:")
            logger.info(f"   Phone: {phone_from_metadata}")
            logger.info(f"   Agent ID: {agent_id_from_meta}")
            logger.info(f"   Webhook context: {webhook_context}")
        except json.JSONDecodeError:
            # Fallback: metadata might be just a plain phone number string
            phone_from_metadata = ctx.job.metadata
            logger.info(f"⚡ Phone from metadata (plain string): {phone_from_metadata}")

    call_start_time = time.time()

    # Check if campaign call
    agent_config = None
    campaign_phone = None  # Store phone for campaign calls
    campaign_participant = None  # Store participant for later use

    if ctx.room.name.startswith('campaign_'):
        logger.info("📞 Campaign call detected, waiting for participant metadata...")
        campaign_participant = await ctx.wait_for_participant()
        logger.info(f"✅ Participant joined: {campaign_participant.identity}")

        try:
            if campaign_participant.metadata:
                logger.info(f"🔍 Participant metadata: {campaign_participant.metadata[:200]}...")
                metadata = json.loads(campaign_participant.metadata)
                if metadata.get('type') == 'campaign':
                    # Extract phone from metadata (campaign_worker sends it)
                    campaign_phone = metadata.get('phone_number')
                    ...
                    # ⭐ Inject phone into prompt for campaign calls
                    if campaign_phone:
                        agent_config['prompt'] += f"\n\n[رقم العميل: {campaign_phone}]"
                        logger.info(f"✅ Campaign phone injected: {campaign_phone}")
                    # Fallback: Try SIP attributes
                    elif campaign_participant.attributes.get('sip.phoneNumber'):
                        campaign_phone = campaign_participant.attributes.get('sip.phoneNumber')
                        agent_config['prompt'] += f"\n\n[رقم العميل: {campaign_phone}]"
                        logger.info(f"✅ Campaign phone from SIP attributes: {campaign_phone}")
    ...
    # ===== GET PHONE FROM SIP ATTRIBUTES (BEFORE SESSION START) =====
    phone_number = None
    participant = None

    # For campaign calls, use campaign_participant (already fetched above)
    if ctx.room.name.startswith('campaign_'):
        participant = campaign_participant
        phone_number = campaign_phone
    else:
        # Wait for participant BEFORE session start (to get phone number)
        logger.info("⏳ Waiting for participant...")
        participant = await ctx.wait_for_participant()

        # Get phone from SIP attributes (LiveKit provides this automatically!)
        phone_number = participant.attributes.get('sip.phoneNumber', '')
        if phone_number:
            agent_config['prompt'] += f"\n\n[رقم العميل: {phone_number}]"
            print (agent_config['prompt'])
            logger.info(f"✅ Phone from SIP attributes injected: {phone_number}")
        else:
            logger.info("ℹ️ No phone number in SIP attributes")
    ...
    # Set SIP info for transcription
    if participant:
