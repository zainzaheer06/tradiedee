...
async def entrypoint(ctx: JobContext):
    ...
    async def init_llm_gemini():
        ...
        return google.realtime.RealtimeModel(
            ...
            model="models/gemini-2.0-flash-exp",  # Or "gemini-2.5-flash-native-audio-preview-09-2025"
            modalities=[Modality.TEXT],
            temperature=0.4,
            
            # Vertex AI config
            vertexai=True,
            project=os.getenv("GOOGLE_CLOUD_PROJECT", "aimeetingassistant-448613"),
            location=os.getenv("GOOGLE_CLOUD_LOCATION", "us-central1"),
            
            # Optional parameters (add if version supports)
            max_output_tokens=200,
            # voice="Puck",  # Not needed in TEXT mode
        )
