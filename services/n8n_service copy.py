...
class N8nService:
    ...
    def _build_http_tool_node(self, name: str, description: str, config: dict, position: list) -> dict:
        """Build an n8n HTTP Request Tool node from a tool config"""
        url = config.get('url', '')
        method = config.get('method', 'GET').upper()
        headers = config.get('headers', {})

        header_params = [{'name': k, 'value': v} for k, v in headers.items()] if headers else []
        ...
        if header_params:
            ...
            node['parameters']['headerParameters'] = {'parameters': header_params}
