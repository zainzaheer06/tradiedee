...
class Assistant(Agent):
    ...
    async def llm_node(self, chat_ctx, tools, model_settings=None):
        """
        🚀 LLM FILTER - Removes JSON, adds fallback
        """
        async def process_stream():
            ...
            async with self.llm.chat(chat_ctx=chat_ctx, tools=tools, tool_choice=None) as stream:
                async for chunk in stream:
                    ...
                    if '{' in content:
                        inside_json = True
                        logger.warning(f"🚫 LLM_BLOCKED: '{content[:40]}'")
                        continue
                    
                    if inside_json:
                        if '}' in content:
                            inside_json = False
                        logger.warning(f"🚫 LLM_BLOCKED (inside): '{content[:40]}'")
                        continue
                    # ================================
            ...
            if not has_content:
                logger.warning("⚠️ No content after filtering, yielding fallback")
                # Create a fake chunk with fallback text
    ...
    async def tts_node(self, text: AsyncIterable[str], model_settings: ModelSettings):
        """
        🚀 TTS FILTER - Blocks JSON, adds fallback to chat
        """
        
        async def process_arabic_text():
            inside_json = False
            has_spoken = False
            blocked_content = []  # Track what was blocked
            ...
            async for chunk in text:
                ...
                if inside_json:
                    blocked_content.append(chunk)
                    if '}' in chunk:
                        inside_json = False
                    logger.warning(f"🚫 TTS_BLOCKED: '{chunk[:40]}'")
                    continue
                # ============================
            ...
            # ===== FALLBACK + ADD TO CHAT =====
            if not has_spoken:
                fallback = "عذراً، ممكن تعيد مرة ثانية؟"
                logger.warning(f"⚠️ Blocked: {''.join(blocked_content)[:60]}")
                logger.info(f"✅ Using fallback: {fallback}")
                
                # Add fallback to chat history (replace error)
                if self._session and hasattr(self._session, 'chat_ctx'):
                    try:
                        # Remove last message if it's the error
                        if self._session.chat_ctx.messages:
                            last_msg = self._session.chat_ctx.messages[-1]
                            if hasattr(last_msg, 'content') and '{' in str(last_msg.content):
                                self._session.chat_ctx.messages.pop()
                                logger.info("🗑️ Removed error from chat history")
                        
                        # Add fallback as assistant message
                        from livekit.agents import llm
                        self._session.chat_ctx.messages.append(
                            llm.ChatMessage(role="assistant", content=fallback)
                        )
                        logger.info("✅ Fallback added to chat history")
                    except Exception as e:
