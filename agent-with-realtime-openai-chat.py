...
class TranscriptionManager:
    ...
    def sync_from_realtime_session(self, realtime_session):
        """
        ✅ Extract conversation from RealtimeSession (OpenAI Realtime API)
        This is called at the end to ensure we have complete conversation
        """
        try:
            conversation = realtime_session.conversation
            logger.info(f"🔍 Accessing RealtimeSession.conversation...")
            
            # Try different ways to get conversation items
            items = []
            if hasattr(conversation, 'items'):
                if callable(conversation.items):
                    items = list(conversation.items())
                else:
                    items = list(conversation.items)
            elif hasattr(conversation, 'get_items'):
                items = list(conversation.get_items())
            elif hasattr(conversation, '_items'):
                items = list(conversation._items)
            else:
                logger.warning("⚠️ Cannot find items in conversation object")
                return
            
            logger.info(f"📊 Found {len(items)} conversation items in RealtimeSession")
            
            for idx, item in enumerate(items):
                try:
                    # Get item ID
                    item_id = getattr(item, 'id', None) or getattr(item, 'item_id', None) or f"item_{idx}"
                    
                    # Skip if already seen
                    if item_id in self.seen_messages:
                        continue
                    ...
                    if not text:
                        text = getattr(item, 'text', '') or getattr(item, 'transcript', '')
                    
                    if text and text.strip():
                        # Use existing add_message method
                        self.seen_messages.add(item_id)
                        msg = {
                            "timestamp": datetime.now(SAUDI_TZ).isoformat(),
                            "role": role,
                            "text": text.strip(),
                        ...
                        self.messages.append(msg)
                        logger.debug(f"   ✅ [{role}] {text[:60]}")
                
                except Exception as e:
                    logger.debug(f"   ⚠️ Error processing item {idx}: {e}")
                    continue
            
            logger.info(f"✅ RealtimeSession sync complete: {len(self.messages)} total messages")
        
        except Exception as e:
            logger.error(f"❌ Error syncing from RealtimeSession: {e}")
            import traceback
            logger.error(traceback.format_exc())
...
class Assistant(Agent):
    ...
    async def tts_node(self, text: AsyncIterable[str], model_settings: ModelSettings):
        """
        🚀 TTS FILTER - Stream with JSON state tracking
        """
        ...
        async def process_arabic_text():
            ...
            async for chunk in text:
                ...
                if inside_json:
                    if '}' in chunk:
                        inside_json = False
                    logger.warning(f"🚫 TTS_BLOCKED (JSON): '{chunk[:40]}'")
                    continue
                # ============================
...
async def entrypoint(ctx: JobContext):
    ...
    realtime_session = llm_model_realtime.session()
    # Create session
    ...
    async def send_call_data():
        ...
        # ===== NEW: Try RealtimeSession as final source =====
        logger.info("🔄 Attempting RealtimeSession extraction...")
        try:
            if hasattr(session, 'llm') and hasattr(session.llm, 'sessions'):
                logger.info(f"🔍 Found {len(session.llm.sessions)} RealtimeSession(s)")
                for idx, realtime_session in enumerate(session.llm.sessions):
                    logger.info(f"📚 Syncing RealtimeSession #{idx}...")
                    transcription.sync_from_realtime_session(realtime_session)
            else:
                logger.warning("⚠️ No RealtimeSession found")
                if hasattr(session, 'llm'):
                    logger.debug(f"   LLM type: {type(session.llm)}")
                    logger.debug(f"   LLM has sessions: {hasattr(session.llm, 'sessions')}")
        except Exception as e:
            logger.error(f"❌ RealtimeSession extraction failed: {e}")
            import traceback
            logger.error(traceback.format_exc())
