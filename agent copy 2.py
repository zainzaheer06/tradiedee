...
async def send_call_data_with_status(status: str, ring_duration: float = 0):
    """Send call data for failed/incomplete calls"""
    try:
        ctx = get_job_context()
        if not ctx:
            return
        ...
        logger.info(f"🚀 Sending webhook for {status} call (rang for {ring_duration:.1f}s)")
        async with aiohttp.ClientSession() as http_session:
            ...
            async with http_session.post(webhook_url, json=payload, timeout=aiohttp.ClientTimeout(total=20)) as resp:
                if resp.status == 200:
                    logger.info(f"✅ Webhook delivered for {status} call")
                else:
...
async def entrypoint(ctx: JobContext):
    ...
    if participant.kind == rtc.ParticipantKind.PARTICIPANT_KIND_SIP:
        ...
        try:
            timeout = 5  # Wait for audio to be ready
    ...
    def on_participant_disconnected(participant: rtc.RemoteParticipant):
        nonlocal inactivity_task, monitor_task, call_ended, call_start_time
        if participant.kind == rtc.ParticipantKind.PARTICIPANT_KIND_SIP:
            # Check if call was answered (any user messages?)
            user_messages = [msg for msg in transcription.messages if msg.get('role') == 'user']
            call_duration = time.time() - call_start_time

            if len(user_messages) == 0 and call_duration < 15:
                # Call disconnected quickly with no user interaction - likely no answer
                logger.warning(f"📴 SIP participant disconnected after {call_duration:.1f}s with no user messages (no answer)")
                asyncio.create_task(send_call_data_with_status('no_answer', call_duration))
            else:
                # Normal disconnect - call was answered
