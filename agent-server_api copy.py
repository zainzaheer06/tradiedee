...
async def entrypoint(ctx: JobContext):
    ...
    # ===== PARALLEL CONNECT + MODEL INIT (for non-campaign rooms) =====
    tts_engine = None
    llm_model_realtime = None

    if not ctx.room.name.startswith('campaign_') and agent_config:
        # We have config pre-loaded - run connect AND models in parallel!
        logger.info("🚀 PARALLEL: Connecting + initializing models simultaneously...")

        async def init_tts_async():
            logger.info("   🔊 Initializing TTS...")
            return create_tts_engine(agent_config)

        async def init_llm_async():
            logger.info("   🧠 Initializing LLM...")
            return create_llm_model(agent_config)
        ...
        _, tts_engine, llm_model_realtime = await asyncio.gather(
            connect_async(),
            init_tts_async(),
            init_llm_async()
        )
        call_start_time = time.time()
        logger.info("✅ PARALLEL complete: Connected + Models ready!")
    else:
    ...
    # ===== OPTIMIZATION 2: Initialize models (only if not already done in parallel) =====
    if tts_engine is None or llm_model_realtime is None:
        logger.info("🚀 Initializing models (campaign/fallback mode)...")
        ...
        # Run initializations in parallel (for campaign/fallback)
        tts_engine, llm_model_realtime = await asyncio.gather(
            init_tts(),
            init_llm()
        )
        logger.info("✅ Models initialized (campaign/fallback mode)!")
    else:
        logger.info("✅ Models already initialized in parallel with connect!")
