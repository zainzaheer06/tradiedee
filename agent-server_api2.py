...
class Assistant(Agent):
    ...
    async def end_call(self, ctx: RunContext):
        """Called when the user wants to end the call. Says a short goodbye then hangs up."""
        try:
            # Short fixed goodbye via TTS only (no LLM generate_reply - avoids timeout)
            goodbye = "مع السلامة، شكراً لاتصالك."
            if getattr(ctx, "session", None) is not None:
                await asyncio.wait_for(ctx.session.say(goodbye), timeout=5.0)
                await asyncio.wait_for(ctx.wait_for_playout(), timeout=6.0)
        except asyncio.TimeoutError:
            logger.warning("⏱️ Goodbye/playout timed out, hanging up anyway")
        except Exception as e:
            logger.debug(f"end_call goodbye: {e}")
        await hangup_call()
        return "Call ended"
...
async def entrypoint(ctx: JobContext):
    ...
    def create_llm_model(config):
        ...
        model_kwargs = {
            ...
            # No language= so transcription preserves what user actually said (Arabic or English)
    ...
    def _extract_text(msg_or_item) -> str:
        """Extract plain text from ChatMessage or conversation item (content may be str or list of parts)."""
        if msg_or_item is None:
            return ""
        text = ""
        if hasattr(msg_or_item, "text_content") and msg_or_item.text_content:
            text = msg_or_item.text_content if isinstance(msg_or_item.text_content, str) else str(msg_or_item.text_content)
        if not text and hasattr(msg_or_item, "content"):
            raw = msg_or_item.content
    ...
    def _register_session_handlers(sess, transcription, agent_config, ctx, state):
        ...
        def on_conversation_item_added(event):
            """Capture every committed user/agent message (reliable for realtime model)."""
            try:
                if not hasattr(event, "item") or event.item is None:
                    return
                item = event.item
                ...
                if not text:
                    return
                # Normalize role for our transcript (assistant -> agent)
                transcript_role = "agent" if role == "assistant" else role
                if transcript_role == "agent" and transcription.greeting_added and text == agent_config.get("greeting"):
                    logger.debug("⏭️ Skipping greeting (already added) in conversation_item_added")
                    return
                transcription.add_message(transcript_role, text, "conversation_item_added")
            except Exception as e:
                logger.debug(f"conversation_item_added: {e}")
        ...
        def on_user_speech_committed(msg: llm.ChatMessage):
            ...
            try:
                ...
                logger.debug(f"user_speech_committed: {e}")
        ...
        def on_agent_speech_committed(msg: llm.ChatMessage):
            ...
            try:
                ...
                logger.debug(f"agent_speech_committed: {e}")
        ...
        async def send_call_data():
            ...
            # Fallback: if events didn't capture turns, try session history (realtime model)
            if len(transcription.messages) <= 1:
                try:
                    history = getattr(sess, "history", None) or (
                        getattr(sess, "chat_ctx", None) and getattr(sess.chat_ctx, "messages", None)
                    )
                    if history and len(history) > 0:
                        logger.info(f"   Backfilling from session history ({len(history)} items)")
                        for msg in history:
                            text = _extract_text(msg)
                            if not text:
                                continue
                            ...
                            if transcript_role == "agent" and transcription.greeting_added and text == agent_config.get("greeting"):
                                continue
                            transcription.add_message(transcript_role, text, "session_history")
                except Exception as e:
                    logger.debug(f"Session history fallback: {e}")
                if len(transcription.messages) <= 1:
