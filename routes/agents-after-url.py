...
def new_agent_form():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('agents.edit_agent_form', agent_num=new_agent.user_agent_number))
...
@agents_bp.route('/<int:agent_num>')
@login_required
@approved_required
def view_agent(agent_num):
    """View agent details and call logs"""
    agent = get_agent_by_user_number(agent_num)

    if not agent:
        flash('Agent not found', 'danger')
        return redirect(url_for('agents.agents_list'))

    # Get call logs with pagination (use internal agent.id for DB queries)
    page = request.args.get('page', 1, type=int)
    per_page = 15  # Show 15 call logs per page

    call_logs_pagination = CallLog.query.filter_by(agent_id=agent.id).order_by(CallLog.created_at.desc()).paginate(
        page=page, per_page=per_page, error_out=False
    )
...
@agents_bp.route('/<int:agent_num>/edit', methods=['GET', 'POST'])
@login_required
@approved_required
def edit_agent_form(agent_num):
    """Edit agent form"""
    agent = get_agent_by_user_number(agent_num)
    ...
    if request.method == 'POST':
        ...
        if not prompt_value:
            flash('Error: Agent prompt cannot be empty. Please create a prompt using the Prompt Builder tab.', 'danger')
            documents = KnowledgeBase.query.filter_by(agent_id=agent.id).order_by(KnowledgeBase.created_at.desc()).all()
        ...
        logger.info(f"🗑️ Invalidating Redis cache for agent {agent.id}")
        redis_service.invalidate_agent_config(agent.id)
        ...
        campaigns_using_agent = Campaign.query.filter_by(agent_id=agent.id).all()

        if campaigns_using_agent:
            logger.info(f"🗑️ Invalidating {len(campaigns_using_agent)} campaign(s) using agent {agent.id}")
        ...
        return redirect(url_for('agents.edit_agent_form', agent_num=agent.user_agent_number))

    # Get knowledge base documents (use internal agent.id for DB queries)
    documents = KnowledgeBase.query.filter_by(agent_id=agent.id).order_by(KnowledgeBase.created_at.desc()).all()
    document_count = len(documents)

    # Get assigned tools
    from models import Tool, AgentTool
    assigned_tool_ids = [at.tool_id for at in AgentTool.query.filter_by(agent_id=agent.id).all()]
...
@agents_bp.route('/<int:agent_num>/delete', methods=['POST'])
@login_required
@approved_required
def delete_agent(agent_num):
    """Delete an agent"""
    agent = get_agent_by_user_number(agent_num)
...
@agents_bp.route('/<int:agent_num>/make-call', methods=['POST'])
@login_required
@approved_required
def make_call_route(agent_num):
...
@agents_bp.route('/<int:agent_num>/knowledge-base')
@login_required
@approved_required
def agent_knowledge_base(agent_num):
    ...
    # Get all knowledge base documents (use internal agent.id)
    documents = KnowledgeBase.query.filter_by(agent_id=agent.id).order_by(KnowledgeBase.created_at.desc()).all()
...
@agents_bp.route('/<int:agent_num>/knowledge-base/upload', methods=['POST'])
@login_required
@approved_required
def upload_knowledge_base(agent_num):
    ...
    for file in files:
        if file and file.filename:
            try:
                ...
                # Add to knowledge base (use internal agent.id for file storage)
                if kb_service.add_document(agent.id, tmp_path, filename):
                    # Add to database
                    kb_doc = KnowledgeBase(
                        agent_id=agent.id,
                        filename=filename,
                        file_path=str(kb_service.get_agent_docs_dir(agent.id) / filename),
    ...
    if uploaded_count > 0:
        ...
        try:
            if kb_service.build_index(agent.id):
                # Update status to ready
                KnowledgeBase.query.filter_by(agent_id=agent.id, status='processing').update({'status': 'ready'})
                db.session.commit()
...
@agents_bp.route('/<int:agent_num>/knowledge-base/<int:doc_id>/delete', methods=['POST'])
@login_required
@approved_required
def delete_knowledge_base_document(agent_num, doc_id):
    ...
    try:
        # Delete from file system (use internal agent.id)
        kb_service.delete_document(agent.id, document.filename)
        ...
        remaining_docs = KnowledgeBase.query.filter_by(agent_id=agent.id).all()
        if remaining_docs:
            KnowledgeBase.query.filter_by(agent_id=agent.id).update({'status': 'ready'})
            db.session.commit()
...
@agents_bp.route('/<int:agent_num>/knowledge-base/rebuild', methods=['POST'])
@login_required
@approved_required
def rebuild_knowledge_base_index(agent_num):
    ...
    try:
        KnowledgeBase.query.filter_by(agent_id=agent.id).update({'status': 'processing'})
        db.session.commit()
    ...
    return redirect(url_for('agents.agent_knowledge_base', agent_num=agent.user_agent_number))


# ==================== AGENT TOOLS MANAGEMENT ====================

@agents_bp.route('/<int:agent_num>/tools', methods=['GET', 'POST'])
@login_required
@approved_required
def agent_tools(agent_num):
    ...
    if request.method == 'POST':
        try:
            ...
            # Remove all existing associations (use internal agent.id)
            deleted_count = AgentTool.query.filter_by(agent_id=agent.id).delete()
            ...
            for tool_id in selected_tool_ids:
                ...
                if tool:
                    ...
                    added_count += 1
                    logger.info(f"Added tool {tool_id} ({tool.name}) to agent {agent.id}")
                else:
            ...
            return redirect(url_for('agents.agent_tools', agent_num=agent.user_agent_number))

    # Get all user's tools
    all_tools = Tool.query.filter_by(user_id=session['user_id']).order_by(Tool.name).all()

    # Get currently assigned tools (use internal agent.id)
    assigned_tool_ids = [at.tool_id for at in AgentTool.query.filter_by(agent_id=agent.id).all()]

    logger.info(f"Agent {agent.id} currently has {len(assigned_tool_ids)} tools assigned: {assigned_tool_ids}")
