...
def agents_list():
    ...
    # HTMX partial response: return only the cards fragment for live search
    is_htmx = request.headers.get('HX-Request')
    if is_htmx:
        return render_template('partials/agent_cards.html', agents=agents, pagination=pagination, search_query=search_query)
...
def new_agent_form():
    ...
    if request.method == 'POST':
        ...
        # Get voice speed value from form (default: 0.90)
        voice_speed = float(request.form.get('voice_speed', 0.90))
        logger.info(f"🔊 CREATE: Received voice_speed from form: {voice_speed}x")
...
def edit_agent_form(agent_id):
    ...
    if request.method == 'POST':
        ...
        if not prompt_value:
            ...
            user_agents = Agent.query.filter(Agent.user_id == agent.user_id, Agent.id != agent_id).order_by(Agent.name).all()
            return render_template('agents/new_agent_form.html', agent=agent, documents=documents, document_count=document_count, user_agents=user_agents)
        ...
        # Update voice speed (TTS speaking rate)
        voice_speed = float(request.form.get('voice_speed', agent.voice_speed or 0.90))
        agent.voice_speed = voice_speed
        logger.info(f"🔊 EDIT: Updated voice_speed from form: {voice_speed}x (agent_id={agent_id})")
        ...
        logger.info(f"🔊 EDIT: Saved to database - agent_id={agent_id}, voice_speed={agent.voice_speed}x")
