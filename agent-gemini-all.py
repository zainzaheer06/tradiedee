...
async def entrypoint(ctx: JobContext):
    ...
    async def init_llm_gemini():
        # Use TEXT modality to work with custom TTS
        return google.beta.realtime.RealtimeModel(
            model="gemini-2.0-flash-exp",
            #Fenrir , Despina, Gacrux
            voice="Kore",  # Gemini voice (not used in text mode, but required)
            temperature=0.4,
    ...
    # Create session - Gemini for LLM, ElevenLabs for voice, Google for STT
    session = voice.AgentSession(
        llm=llm_model_gemini,
        #tts=tts_engine,
    ...
    await session.generate_reply(
            instructions="Just say Assalaamu Alaikum in  Arabic: السلام عليكم ورحمة الله وبركاته"
        )

    # Send greeting immediately - no sleeps!
    #await session.generate_reply(greeting_message, allow_interruptions=True)
