...
async def entrypoint(ctx: JobContext):
    ...
    async def init_llm():
        ...
        logger.info(f"🌡️ LLM Temperature from agent: {agent_config.get('temperature')}")
        ...
        #   - 'dynamic'  : Server VAD - fast, silence-based detection (quick responses)
        # ============================================================
        vad_mode = agent_config.get('vad_mode', 'dynamic')
        logger.info(f"🎙️ VAD Mode: {vad_mode}")

        # Build turn_detection based on vad_mode
        if vad_mode == 'precise':
            # No VAD - waits for complete input
            turn_detection_config = None
            logger.info("🎙️ VAD: Disabled (Precise mode - best for number collection)")
            ...
            logger.info("🎙️ VAD: Server (Dynamic mode - fast responses)")
