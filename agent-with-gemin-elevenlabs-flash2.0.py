...
from livekit.plugins import noise_cancellation, openai, elevenlabs, google, silero
...
def build_full_prompt(user_prompt: str) -> str:
    ...
    """
    # Arabic-focused prompt optimized for Gemini tool calling
    system_instructions = """
You are a polite and helpful Saudi Arabic voice assistant built by Nevox AI.
...
if ask for end call or something then use end_call tool to end the call politely.
"""
...
class Assistant(Agent):
    def __init__(self, config: dict = None, agent_id: int = None, preloaded_tools: list = None) -> None:
        ...
        # Load dynamic tools BEFORE initializing parent Agent
        ...
        # CRITICAL FOR GEMINI: Dynamically bind tools as instance methods BEFORE super().__init__()
        # Gemini discovers tools by scanning instance methods, not from a tools parameter
        if dynamic_tools:
            for tool_func in dynamic_tools:
                if hasattr(tool_func, '__name__'):
                    # Bind the function to this instance as a method
                    # The function is already decorated with @function_tool from tool_service
                    setattr(self, tool_func.__name__, tool_func)
                    logger.info(f"🔧 Bound dynamic tool as method: {tool_func.__name__}")

        # CRITICAL: Create LLM and TTS INSIDE the Agent class, not passed from outside!
        # This is the pattern that works in the sample code
        super().__init__(
            ...
            #llm = google.inference.LLM( # NOTE: Use LLM, not RealtimeModel
            #    model="gemini-2.5-flash", 
            #    # Do NOT set modalities here. The LLM object is inherently text-focused
            #    # and designed to integrate with separate TTS/STT plugins.
            #    temperature=0.4,
            #)
            tts=elevenlabs.TTS(
        ...
        if dynamic_tools:
            logger.info(f"🔧 {len(dynamic_tools)} custom tools registered")
            logger.debug(f"   Tool names: {[t.__name__ if hasattr(t, '__name__') else str(t) for t in dynamic_tools]}")
...
async def entrypoint(ctx: JobContext):
    ...
    async def init_llm_gemini():
        # Use standard Gemini model (NOT beta) to work with custom TTS
        # Note: Tools are managed at the Agent level, not the LLM level for Gemini
        return google.realtime.RealtimeModel(
            model="models/gemini-2.0-flash-exp",
            modalities=[Modality.TEXT],
            temperature=0.6,  # Higher temperature for better tool calling
        )
    ...
    # Run all initializations in parallel - Gemini LLM + Custom TTS/STT
    tts_engine, stt_engine_google, llm_model_gemini = await asyncio.gather(
        init_tts(),
        init_stt(),
        init_llm_gemini()
    )

    # Log tools that will be available to the agent
    if dynamic_tools:
        logger.info(f"🔧 {len(dynamic_tools)} custom tools will be available to Gemini via Agent layer")

    logger.info("✅ All models initialized (Gemini + ElevenLabs TTS + Google STT)!")

    # Create session WITHOUT passing LLM/TTS (they go in Agent's __init__)
    # This is the correct pattern for tool calling with Gemini
    ...
    await session.start(
        ...
        agent=Assistant(
            ...
            # LLM and TTS are created inside Assistant.__init__(), not passed as parameters
        ),
    ...
    # Use generate_reply to properly initialize the LLM with tool context (NOT say!)
    # This ensures the LLM is aware of available tools from the start
    await session.generate_reply(
        instructions=f"just say this in arabic: {greeting_message}"
    )
    logger.info("✅ Greeting sent via LLM with tool context initialized!")
