...
# Import contact collection tasks
from contact_tasks import (
    ContactCollectionGroup,
    collect_contact_information,  # All: name, phone, email
    collect_name_and_email,        # Just name + email
    collect_name_and_phone,        # Just name + phone
    collect_name_only,             # Just name
    collect_phone_only,            # Just phone
    collect_email_only             # Just email
)
...
- Ask for name phone number and email from user
...
ABSOLUTE RULE #1 - NEVER GENERATE JSON 

You are a VOICE assistant. You ONLY speak natural Arabic sentences.

UNDER NO CIRCUMSTANCES should you EVER generate:
- JSON like {"error": "..."}
- Code like {"status": "..."}
- Technical syntax like {key: value}
- Error messages in JSON format

If you are confused, uncertain, or don't understand:
→ Say in Arabic: "عذراً، ما فهمت. ممكن تعيد؟"
→ Say in Arabic: "ممكن توضح أكثر؟"
→ Say in Arabic: "آسف، ما فهمت السؤال. تقدر تعيده بطريقة ثانية؟"

NEVER EVER respond with {"error": "..."} or any JSON.
NEVER EVER generate technical error messages.
ONLY natural spoken Arabic sentences.

This rule applies even if:
- User speaks mixed Arabic/English
- User mispronounces words
- You don't understand the question
- You detect "language issues"
- ANYTHING confuses you

ALWAYS respond with natural Arabic, NEVER JSON.
END OF ABSOLUTE RULE #1 
...
NEVER call end_call() when:
- You don't understand the user's question
- User speaks unclear Arabic
- You detect "language mismatch" or confusion
- User speaks mixed Arabic/English
- You are uncertain about anything
-If confused → Ask for clarification in Arabic
...
class Assistant(Agent):
    ...
    @function_tool
    async def collect_contact_info(self, ctx: RunContext) -> str:
        """Collect ALL customer information: name, phone, and email (all three)"""
        try:
            logger.info("🎯 Starting FULL contact collection (name + phone + email)...")

            # Collect ALL: name, phone, email
            contact_info = await collect_contact_information(chat_ctx=self.chat_ctx)

            # Store in session userdata
            ctx.session.userdata.contact_info = contact_info

            logger.info(f"✅ Full contact collected: {contact_info['name']} - {contact_info['phone']} - {contact_info['email']}")

            # TODO: Save to database here
            # save_lead_to_database(contact_info)

            return f"تم تسجيل معلومات العميل: {contact_info['name']}"

        except Exception as e:
            logger.error(f"❌ Error collecting contact info: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return "عذراً، في مشكلة بتسجيل المعلومات"

    @function_tool
    async def collect_name_email_only(self, ctx: RunContext) -> str:
        """Collect ONLY name and email (skip phone number)"""
        try:
            logger.info("🎯 Collecting name + email only (no phone)...")

            # Collect just name and email
            contact_info = await collect_name_and_email(chat_ctx=self.chat_ctx)

            ctx.session.userdata.contact_info = contact_info
            logger.info(f"✅ Name + Email collected: {contact_info['name']} - {contact_info['email']}")

            return f"تم تسجيل الاسم والإيميل: {contact_info['name']}"
    ...
    @function_tool
    async def collect_name_phone_only(self, ctx: RunContext) -> str:
        """Collect ONLY name and phone (skip email)"""
        try:
            logger.info("🎯 Collecting name + phone only (no email)...")

            # Collect just name and phone
            contact_info = await collect_name_and_phone(chat_ctx=self.chat_ctx)

            ctx.session.userdata.contact_info = contact_info
            logger.info(f"✅ Name + Phone collected: {contact_info['name']} - {contact_info['phone']}")

            return f"تم تسجيل الاسم ورقم الجوال: {contact_info['name']}"
    ...
    @function_tool
    async def collect_just_name(self, ctx: RunContext) -> str:
        """Collect ONLY the customer's name (no phone, no email)"""
        try:
            logger.info("🎯 Collecting name only...")

            # Collect just name
            contact_info = await collect_name_only(chat_ctx=self.chat_ctx)

            ctx.session.userdata.contact_name = contact_info['name']
            logger.info(f"✅ Name collected: {contact_info['name']}")

            return f"تم تسجيل الاسم: {contact_info['name']}"

        except Exception as e:
            logger.error(f"❌ Error: {e}")
            return "عذراً، في مشكلة بتسجيل الاسم"
...
async def entrypoint(ctx: JobContext):
    ...
    def on_participant_disconnected(participant: rtc.RemoteParticipant):
        ...
        if participant.kind == rtc.ParticipantKind.PARTICIPANT_KIND_SIP:
            ...
            # ✅ SEND WEBHOOK FIRST, THEN HANG UP
