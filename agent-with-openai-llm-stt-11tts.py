...
# Import recording service (dual-voice with mixing)
...
def get_default_config1():
    return {
        ...
        'prompt': """أنت نورا من نيفوكس، مندوبة مبيعات لحلول الذكاء الاصطناعي الصوتي.

## الأسلوب
- سعودي ودود ومحترف
- مختصرة (1-2 جمل)
- تعبيرات: أبشر، تمام، يسعدني، ولا يهمك

## المنتج
نيفوكس: موظف صوتي ذكي 24/7 بلهجة سعودية
- يرد على المكالمات والحجوزات
- يقلل التكلفة 70%
- بدون انتظار للعملاء

## العملاء المناسبين
مطاعم، عيادات، عقارات، متاجر إلكترونية

## الخطوات
1. بعد السلام: "أنا نورا من نيفوكس، مساعدة صوتية بالذكاء الاصطناعي. معك دقيقتين أشرح لك كيف نقدر نساعد نشاطك التجاري؟"

2. اسأل عن نوعية النشاط واشرح الفائدة:
   - مطاعم: "نيفوكس يستقبل طلبات التوصيل والحجوزات تلقائياً ٢٤ ساعة، وبهذا ما يفوتك أي طلب أبداً"
   - عيادات: "يحجز المواعيد ويذكّر المرضى، وفريقك يركز على الرعاية"
   - عام: "موظف يشتغل ٢٤/٧ بلهجة سعودية طبيعية وبتكلفة أقل من موظف واحد"

3. إذا مهتم: اطلب جوال وإيميل وحدد موعد Demo

4. إذا مشغول: "متى وقت مناسب أتصل فيك؟"

5. إذا مو مهتم: شكر واستخدم end_call

## أسئلة شائعة
- كم السعر؟ "حسب احتياجاتك، ممكن أسوي عرض بعد ما أفهم نشاطك"
- طبيعي؟ "أيوه، تقدر تجرب demo مجاني"
- معقد؟ "أبداً، فريقنا يساعدك وجاهز بأيام"
- عندي موظفين؟ "نيفوكس يساعد فريقك، يرد على الأسئلة البسيطة وفريقك يركز على المهم"
...
def build_full_prompt1(user_prompt: str) -> str:
    """Optimized system instructions"""
    
    system_instructions = """أنت مساعد صوتي سعودي من نيفوكس.

القواعد:
- رد بالعربي السعودي (1-2 جمل)
- استخدم الأدوات بصمت (لا تقرأ JSON)
- بعد "السلام عليكم" → مباشرة عرّف نفسك
- عند الوداع → استخدم end_call
- إذا رد آلي → اترك رسالة واستخدم end_call

لا تذكر: OpenAI، JSON، أكواد
"""
...
class Assistant(Agent):
    def __init__(self, config: dict = None, agent_id: int = None, preloaded_tools: list = None, webhook_context: dict = None, room_name: str = None) -> None:
        if config is None:
        ...
        self.room_name = room_name  # Store room name for end_call
    ...
    @function_tool
    async def test_tool(self, message: str):
        """Test tool to verify tool calling works. Call this with any message."""
        logger.info(f"🎯 TEST TOOL CALLED: {message}")
    ...
    async def end_call(self, ctx: RunContext):
        """
        Called when user wants to end the call
        Uses say() to avoid breaking prompt cache
        """
        ...
        # Stop recording before hanging up
        if self.room_name:
            logger.info("🛑 Stopping recording before call end")
            await audio_recorder.stop_recording(self.room_name)
...
async def entrypoint(ctx: JobContext):
    ...
    async def init_stt():
        return openai.STT(
            ...
            turn_detection={
                "type": "server_vad",
               "threshold": 0.45,  # ✅ Already good
                "create_response":True,
                "interrupt_response":True,
                
                # ❌ These are too high, causing 5.46s delay!
                "prefix_padding_ms": 250,    # ⬇️ Reduce from 300ms
                "silence_duration_ms": 350,  # ⬇️ Reduce from 250ms
            },
    ...
    '''    
    async def init_llm():
        """OpenAI GPT-4o LLM (not Realtime)"""
        return openai.LLM(
            model="gpt-4.1-nano-2025-04-14",
        )
    '''
    async def init_llm5():
        """Use GPT-5-mini - Latest and fastest!"""
        return openai.LLM(
            model="gpt-5-mini-2025-08-07",  # ✅ Specific snapshot
            # OR use:
            # model="gpt-5-mini",            # ✅ Latest version (auto-updates)
    ...
    async def init_llm():
        return openai.LLM(
            #model="gpt-4o", gpt-4.1-mini,gpt-4o-mini,gpt-4.1-nano,gpt-5-mini-2025-08-07
            model="gpt-4o-mini", 
            temperature=0.4,
            max_completion_tokens=50,
            tool_choice="auto",           # Enable automatic tool selection
            parallel_tool_calls=True,     # Allow multiple tool calls
            # ✅ UNIQUE CACHE KEY PER AGENT
            prompt_cache_key=f"nevox-agent-{agent_id}-v1",  # ✅ Each agent gets own cache!

        )
    async def init_llm_google():
        return google.LLM(
            ...
            #- gemini-2.5-flash-native-audio-preview-09-2025    # Latest
            #- gemini-2.5-flash-preview-native-audio-dialog     # Dialog optimized
            #- gemini-2.5-flash-exp-native-audio-thinking-dialog # With thinking
            #
            #gemini-3-flash-preview, gemini-2.0-flash-exp, gemini-2.5-flash
    ...
    async def init_llm_groq():
        """Groq LLM with proper syntax"""
        return groq.LLM(
            model="llama-3.3-70b-versatile",
            api_key=os.environ.get("GROQ_API_KEY"),
            temperature=0.4,
            tool_choice="auto",          # ✅ String, not bare word
            #parallel_tool_calls=True,    # ✅ Python boolean (capital T)
        )   
          # ✅ CORRECT: Gemini Live API in TEXT mode
    realtime_model = google.realtime.RealtimeModel(
        model="gemini-2.5-flash-native-audio-preview-12-2025",
        
        # Use AUDIO mode (default)
        modalities=[Modality.AUDIO],  # ✅ Keep default
        output_audio_transcription=True,
        
        voice="Puck",  # Required for AUDIO mode
        temperature=0.4,
    )
       
    # Run all initializations in parallel
    # Run all initializations in parallel
    tts_engine, stt_engine, llm_model = await asyncio.gather(
        init_tts(),
        init_stt(),
        #init_llm_google(),
        init_llm()
    ...
    logger.info("✅ All models initialized (OpenAI STT + LLM, ElevenLabs TTS)")
    ...
    session = voice.AgentSession(
        ...
        #vad=silero.VAD.load(
        #    min_silence_duration=0.07,      # ⬇️ Reduce from 0.1
        #    min_speech_duration=0.03,      # ✅ Already optima  0.03
        #    activation_threshold=0.35,     # ⬇️ Lower from 0.35
        #),
        #vad=silero.VAD.load(
        #    min_silence_duration=0.02,      # More aggressive
        #    min_speech_duration=0.02,      # Keep low
        #    activation_threshold=0.2,       # More sensitive
        #),
        #llm=realtime_model,
        stt=stt_engine,
        llm=llm_model,
    ...
    async def send_call_data():
        ...
        # Wait for recording upload to complete (background upload takes ~10-20 seconds)
        recording_url = None
        for attempt in range(50):  # Wait up to 25 seconds for upload
        ...
        try:
            payload = {
                ...
                'recording_url': recording_url  # Include Alibaba OSS URL
            }
            if recording_url:
                logger.info(f"📼 Recording URL: {recording_url[:80]}...")
            else:
                logger.warning(f"⚠️ Recording URL not available yet (upload still in progress)")
    ...
    await session.start(
        ...
        agent=Assistant(
            ...
            webhook_context=webhook_context,  # Pass webhook data to agent
            room_name=ctx.room.name  # Pass room name for recording
        ),
    ...
    def on_participant_disconnected(participant: rtc.RemoteParticipant):
        ...
        if participant.kind == rtc.ParticipantKind.PARTICIPANT_KIND_SIP:
            ...
            # ✅ STOP RECORDING, SEND WEBHOOK, THEN HANG UP
