...
class Assistant(Agent):
    def __init__(self, config: dict = None, agent_id: int = None, preloaded_tools: list = None, webhook_context: dict = None) -> None:
        ...
        if webhook_context:
            ...
            webhook_prompt = "\n\n## معلومات العميل من CRM:\n"
            for key, value in webhook_context.items():
                webhook_prompt += f"- {key}: {value}\n"
            webhook_prompt += "\nاستخدم هذه المعلومات في المحادثة بطريقة طبيعية.\n"
...
async def entrypoint(ctx: JobContext):
    ...
    # ===== NON-BLOCKING PRE-CALL WEBHOOK FETCH =====
    ...
    # Start webhook fetch if enabled (NON-BLOCKING)
    if agent_id and agent_config.get('workflow_id'):
        async def fetch_and_inject_webhook_data():
            """Fetch webhook data and inject into live conversation"""
            try:
                ...
                if workflow_row and workflow_row['pre_call_enabled'] and workflow_row['is_active']:
                    logger.info(f"🔍 Fetching webhook data (non-blocking)...")
                    ...
                    webhook_data = await asyncio.to_thread(
                        fetch_pre_call_data_sync,
                    ...
                    if webhook_data:
                        ...
                        context_str += "\nاستخدم هذه المعلومات في المحادثة بشكل طبيعية ومخصصة.\n"

                        # Build full injection prompt
                        full_prompt = f"لقد تم تحميل معلومات العميل الآن. {context_str}\n\nاستمر في المحادثة بشكل طبيعي واستخدم هذه المعلومات عند الحاجة."

                        # ===== LOG 3: Show injection prompt =====
                        logger.info("=" * 80)
                        logger.info("💉 INJECTING INTO LLM CONVERSATION:")
                        ...
                        await session.generate_reply(instructions=full_prompt)
                        logger.info("✅ Webhook data injected successfully!")
                        return webhook_data
                    else:
                        logger.warning("⚠️ Webhook returned empty")
                        return None
                else:
                    logger.debug("ℹ️ Pre-call webhook not enabled")
                    return None
            except Exception as e:
        ...
        # Start fetch task (fire and forget - doesn't block)
        webhook_fetch_task = asyncio.create_task(fetch_and_inject_webhook_data())
        logger.info("🚀 Non-blocking webhook fetch started...")
    ...
    await session.start(
        room=ctx.room,
        agent=Assistant(config=agent_config, agent_id=agent_id, preloaded_tools=dynamic_tools, webhook_context=None),
