...
# IMPORTANT TOOLS:
You have access to these function tools - USE THEM!

1. get_weather(location: str) - Get weather information
   When user asks "ما الطقس؟" or "what's the weather?" → Call get_weather(location)
   Example: User says "ما هو الطقس في الرياض؟" → Call get_weather("الرياض")
   
2. end_call() - End the phone call
   When user says "وداعاً" or "مع السلامة" or "شكراً" → Call end_call()
   MUST call this tool when user wants to hang up!
...
## ABSOLUTELY FORBIDDEN - NEVER SAY OUT LOUD:
Function names: end_call, query, search, hangup, detected_answering_machine
Technical terms: error, exception, null, undefined, callback, response, request
Code/JSON: {}, [], (), "key": "value", function_name()
English tech words: API, JSON, HTTP, async, await, params, payload

## CORRECT BEHAVIOR:
Call tools SILENTLY - only speak natural results
- If error: Say "عذراً، دعني أحاول مرة أخرى" (NOT "error occurred")
- If ending: Say "مع السلامة" (NOT "end_call executed")
No narration of internal processes

## EXAMPLES:
WRONG: "calling end_call function"
RIGHT: "مع السلامة وشكراً لك"

WRONG: "error: query failed"  
RIGHT: "عذراً، دعني أحاول مرة أخرى"

WRONG: "{name: 'test'}"
RIGHT: [just give natural response]

REMEMBER: This is a PHONE CONVERSATION. Only say what sounds natural when SPOKEN.
...
class Assistant(Agent):
    def __init__(self, config: dict = None, agent_id: int = None, preloaded_tools: list = None, llm_model=None, tts_engine=None,stt_engine=None) -> None:
        if config is None:
        ...
        super().__init__(
            ...
            llm=llm_model,        # ✅ ADD THIS
            tts=tts_engine,       # ✅ ADD THIS
        )
...
async def entrypoint(ctx: JobContext):
    ...
    async def init_llm_g():
        return google.realtime.RealtimeModel(
            model="models/gemini-2.0-flash-exp",  # Latest experimental
    ...
    await session.start(
        room=ctx.room,
        agent=Assistant(config=agent_config, agent_id=agent_id, preloaded_tools=dynamic_tools, llm_model=llm_model_realtime,tts_engine=tts_engine),  # ✅ Pass LLM to Agent          # ✅ Pass TTS to Agent),
        room_input_options=RoomInputOptions(
            noise_cancellation=noise_cancellation.BVCTelephony(),
        ),
    )
    # ✅ Add this debug check:
    agent_instance = session._agent
    if hasattr(agent_instance, 'tools'):
        tool_names = [getattr(t, '__name__', str(t)) for t in agent_instance.tools]
        logger.info(f"🔧 Registered tools: {tool_names}")
        logger.info(f"🔧 Total tools: {len(agent_instance.tools)}")
    else:
        logger.warning("⚠️ No tools found on agent!")
