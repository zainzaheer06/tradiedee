...
# SSRF Protection - Allowed webhook domains
...
def is_safe_webhook_url(url: str) -> tuple[bool, str]:
    """
    Validates webhook URL for SSRF protection.
    
    Checks:
    - Must use HTTPS (encrypts webhook data in transit, prevents MITM attacks)
    - Must be from allowed domains only (prevents SSRF to internal services)
    ...
    try:
        ...
        if parsed.scheme != 'https':
            return False, 'Webhook URL must use HTTPS for security'
        ...
        if not is_allowed:
            allowed_list = ', '.join(ALLOWED_WEBHOOK_DOMAINS)
            return False, f'Webhook URL domain not allowed. Allowed domains: {allowed_list}'
        
        return True, ''
        
    except Exception as e:
        return False, f'Invalid webhook URL format: {str(e)}'
...
def create_workflow():
    ...
    try:
        ...
        is_valid, error_message = is_safe_webhook_url(webhook_url)
        if not is_valid:
...
def delete_workflow(workflow_id):
    ...
    try:
        ...
        # Delete related webhook logs first (to avoid foreign key constraint error)
        deleted_logs = WebhookLog.query.filter_by(workflow_id=workflow_id).delete()
        if deleted_logs:
            logger.info(f"Deleted {deleted_logs} webhook log(s) for workflow {workflow_id}")
