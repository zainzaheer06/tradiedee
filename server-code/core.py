...
from models import db, User, Agent, CallLog, Tool, AgentTool, InboundConfiguration, Campaign, CampaignContact,  Workflow, SAUDI_TZ
...
def call_logs():
    ...
    filter_call_type = request.args.get('call_type', '')
    filter_interest = request.args.get('interest', '')
    filter_agent = request.args.get('agent', '', type=str)
    ...
    # Apply status filter
    if filter_status:
        query = query.filter(CallLog.status == filter_status)

    # Apply call type filter
    if filter_call_type:
        query = query.filter(CallLog.call_type == filter_call_type)

    # Apply agent filter
    if filter_agent:
        query = query.filter(CallLog.agent_id == filter_agent)

    # Apply phone search
    if search_phone:
        query = query.filter(
            db.or_(
                CallLog.to_number.contains(search_phone),
                CallLog.from_number.contains(search_phone)
            )
        )

    # Apply interest filter (requires checking sentiment_summary JSON)
    if filter_interest:
        if filter_interest == 'interested':
            query = query.filter(CallLog.sentiment_summary.like('%"interest": "Interested"%'))
        elif filter_interest == 'not_interested':
            query = query.filter(CallLog.sentiment_summary.like('%"interest": "Not Interested"%'))
        elif filter_interest == 'na':
            # No Answer or Failed calls (no interest data)
...
def call_ended_webhook():
    ...
    try:
                    ...
                    if campaign.agent_id and contact.status == 'completed':
                        ...
                        if agent and agent.workflow_id:
                            ...
                            if workflow and workflow.is_active:
                                ...
                                # Trigger webhook (NON-BLOCKING - returns immediately)
        ...
        if not call_log:
            ...
            # Extract SIP info
            sip_info = metadata.get('sip_info', {})
            from_number = sip_info.get('phone_number', 'web')
            to_number = sip_info.get('trunk_phone_number', 'web')

            # Find inbound configuration by phone number called (to_number)
            # Use flexible phone number matching (same logic as agent-inbound.py)
            inbound_config = None
            agent = None

            if to_number and to_number != 'web':
                clean_to_number = to_number.replace('+', '').replace('-', '').replace(' ', '')
                all_configs = InboundConfiguration.query.all()
                for config in all_configs:
                    if config.phone_number:
                        clean_config_number = config.phone_number.replace('+', '').replace('-', '').replace(' ', '')
                        if clean_to_number and (clean_to_number.endswith(clean_config_number) or clean_config_number.endswith(clean_to_number)):
                            inbound_config = config
                            agent = config.agent
                            logger.info(f"Matched TO number {to_number} -> Config: {config.name}")
                            break

            # Fallback: Extract agent_id from room name for NON_SIP calls (e.g., call-113-test-xxx)
            if not inbound_config:
                try:
                    room_parts = room_name.split('-')
                    if len(room_parts) >= 2 and room_parts[0] == 'call':
                        agent_id_from_room = int(room_parts[1])
                        agent = db.session.get(Agent, agent_id_from_room)
                        if agent:
                            logger.info(f"NON_SIP call: Using agent from room name: {agent.name} (ID: {agent_id_from_room})")
                except (IndexError, ValueError) as e:
                    logger.warning(f"Could not extract agent_id from room name: {room_name}")

            if inbound_config or agent:
                # Use inbound_config user if available, otherwise use agent's user
                user_id = inbound_config.user_id if inbound_config else agent.user_id
                ...
                config_name = inbound_config.name if inbound_config else 'NON_SIP'
                logger.info(f"Created inbound call log (ID: {call_log.id}) for config: {config_name}, agent: {agent.name if agent else 'None'}")
            else:
                logger.error(f"No inbound configuration or agent found for: {to_number}, room: {room_name}")
            ...
            if inbound_config:
                ...
                logger.error(f"L No inbound configuration found for phone number: {to_number}")
                return jsonify({'error': f'No inbound configuration for {to_number}'}), 404
            '''
        ...
        if call_log:
            ...
            user_messages = [msg for msg in messages if msg.get('role') == 'user']
            sip_info = metadata.get('sip_info', {})
            sip_call_status = sip_info.get('call_status', 'web')
