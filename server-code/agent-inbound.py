        ...
        with get_readonly_session() as session:
            ...
            for ic in configs:
                rows.append({
                    ...
                    'transfer_targets': ic.agent.transfer_targets or None,
                })
            ...
            if row['phone_number']:
                ...
                if clean_to_number and (clean_to_number.endswith(clean_config_number) or clean_config_number.endswith(clean_to_number)):
                    config = {
                        ...
                        'transfer_targets': row['transfer_targets'],
                    }
        ...
        if self.is_transferred:
            ...
            try:
                ...
                if hasattr(self.session, '_tts'):
                    ...
                    logger.info(f"🔊 TTS voice updated to: {self.config.get('voice_name')} ({self.config.get('voice_id')})")
            ...
            if self.background_audio and os.path.exists(chime_path):
                try:
                    ...
                    logger.info("⏸️ Fallback pause (2)...")
...
async def entrypoint(ctx: JobContext):
    ...
    async def init_tts():
        return build_tts_engine(agent_config)
