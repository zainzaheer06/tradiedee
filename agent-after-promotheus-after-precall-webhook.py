    ...
    try:
        ...
        if row:
            config = {
                ...
                'vad_mode': row['vad_mode'] if row['vad_mode'] else 'dynamic'  # VAD mode: precise, natural, dynamic
            }
...
async def entrypoint(ctx: JobContext):
    ...
    async def init_llm():
        ...
        # ============================================================
        # VAD MODE CONFIGURATION (using **kwargs method)
        # ============================================================
        # Get VAD mode from agent config (default: 'dynamic')
        # Options:
        #   - 'precise'  : No turn_detection param - API uses default (best for numbers)
        #   - 'natural'  : Semantic VAD - AI detects natural turn completion (balanced)
        #   - 'dynamic'  : Server VAD with short silence (250ms) - fast responses
        # ============================================================
        ...
        if vad_mode == 'precise':
            ...
            logger.info("🎙️ VAD: Not set (Precise mode - API default, best for number collection)")
            ...
            logger.info("🎙️ VAD: Semantic (Natural mode - AI-based turn detection)")
            ...
            logger.info("🎙️ VAD: Server with 250ms silence (Dynamic mode - fast responses)")
