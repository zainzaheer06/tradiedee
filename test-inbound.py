...
async def entrypoint(ctx: JobContext):
    ...
    tts_engine = elevenlabs.TTS(
        ...
        voice_settings=elevenlabs.VoiceSettings(
            ...
            speed=1.0,  # Maximum speed for faster greeting
            use_speaker_boost=True
        ),
        streaming_latency=0,  # 0 = Maximum speed (critical for greeting!)
    ...
    llm_model = RealtimeModel(
        ...
        turn_detection=TurnDetection(
            ...
            prefix_padding_ms=100,  # Reduced from 300ms → faster response
            silence_duration_ms=300,  # Reduced from 500ms → faster detection
    ...
    # --- Initial greeting ---
    # IMPORTANT: Add small delay to let audio streams fully connect
    # Without this, user hears silence and hangs up before greeting plays
    logger.info("⏳ Waiting for audio streams to connect...")
    await asyncio.sleep(0.5)  # 500ms delay for audio connection
    ...
    await session.say(greeting_message, add_to_chat_ctx=False)  # Don't add greeting to conversation history
...
if __name__ == "__main__":
    ...
    logger.info(f"   Agent name: nevox-inbound-agent (explicit dispatch)")
