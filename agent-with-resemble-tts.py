...
from livekit.plugins import noise_cancellation, openai, elevenlabs, google, resemble
...
async def entrypoint(ctx: JobContext):
    ...
    async def init_tts():
        # Use Resemble AI voice from .env or agent config
        resemble_voice_id = os.getenv('RESEMBLE_VOICE_ID', agent_config.get('voice_id', '37fcfeb8'))
        return resemble.TTS(
            voice_uuid=resemble_voice_id,
            sample_rate=24000,  # Optimized for voice calls (lower than default 44100)
            use_streaming=True,  # Enable streaming for lower latency
        )
    ...
    async def init_llm():
        return RealtimeModel(
            #model="gpt-4o-realtime-preview", for this 150ms, 250ms
