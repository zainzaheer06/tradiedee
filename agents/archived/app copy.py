...
def dashboard():
    ...
    return render_template('user_dashboard.html', user=user, agents=agents, recent_calls=recent_calls, total_calls=total_calls, datetime=datetime)
...
def trunk_management():
    ...
    return render_template('admin_trunk_management.html', users=users)
...
def configure_user_trunk(user_id):
    ...
    return render_template('admin_configure_trunk.html', user=user)
...
def inbound_trunk_management():
    """View and manage inbound SIP trunk configuration for agents"""
    # Get all inbound agents from all users
    inbound_agents = Agent.query.filter_by(call_type='inbound').order_by(Agent.created_at.desc()).all()
    return render_template('admin_inbound_trunk_management.html', agents=inbound_agents)

@app.route('/admin/configure-inbound-trunk/<int:agent_id>', methods=['GET', 'POST'])
@login_required
@admin_required
def configure_inbound_trunk(agent_id):
    """Configure inbound SIP trunk for a specific agent"""
    agent = Agent.query.get_or_404(agent_id)

    if request.method == 'POST':
        inbound_trunk_id = request.form.get('inbound_trunk_id', '').strip()

        # Update agent trunk configuration
        agent.inbound_trunk_id = inbound_trunk_id if inbound_trunk_id else None
        db.session.commit()

        if inbound_trunk_id:
            flash(f'Inbound trunk "{inbound_trunk_id}" configured for agent "{agent.name}"', 'success')
        else:
            flash(f'Inbound trunk removed from agent "{agent.name}"', 'success')

        return redirect(url_for('inbound_trunk_management'))

    return render_template('admin_configure_inbound_trunk.html', agent=agent)

@app.route('/admin/remove-inbound-trunk/<int:agent_id>', methods=['POST'])
@login_required
@admin_required
def remove_inbound_trunk(agent_id):
    """Remove inbound SIP trunk from agent"""
    agent = Agent.query.get_or_404(agent_id)

    agent.inbound_trunk_id = None
    db.session.commit()

    flash(f'Inbound trunk removed from "{agent.name}"', 'success')
    return redirect(url_for('inbound_trunk_management'))
...
def agents_list():
    ...
    return render_template('agents_list.html', agents=agents, pagination=pagination)
...
def new_agent_form():
    ...
    return render_template('new_agent_form.html')
...
def edit_agent_form(agent_id):
    ...
    return render_template('new_agent_form.html', agent=agent, documents=documents, document_count=document_count)
...
def view_agent(agent_id):
    ...
    return render_template('view_agent.html', agent=agent, call_logs=call_logs_pagination.items, pagination=call_logs_pagination)
...
def web_call():
    ...
    user = db.session.get(User, session['user_id'])
    return render_template('web_call.html', user=user)
...
def subscription():
    user = db.session.get(User, session['user_id'])
    return render_template('subscription.html', user=user, datetime=datetime)
...
def view_call_log(log_id):
    ...
    return render_template('view_call_log.html', log=log)
...
def create_inbound_agent():
    """Create new inbound agent by selecting existing agent"""
    # Get all outbound agents to select from (generic agents)
    agents = Agent.query.filter_by(
        user_id=session['user_id'],
        call_type='outbound'
    ...
    if request.method == 'POST':
        ...
        if not selected_agent:
            ...
            return render_template('create_inbound_agent.html', agents=agents)

        # Calculate next user-specific agent number
        max_agent = db.session.query(db.func.max(Agent.user_agent_number)).filter_by(user_id=session['user_id']).scalar()
        next_agent_number = (max_agent or 0) + 1

        # Create new inbound agent with selected agent's configuration
        inbound_agent = Agent(
            user_id=session['user_id'],
            user_agent_number=next_agent_number,
            name=name,
            prompt=selected_agent.prompt,
            greeting=selected_agent.greeting,
            voice_id=selected_agent.voice_id,
            voice_name=selected_agent.voice_name,
            call_type='inbound',
            phone_number=phone_number,
            dispatch_rule_id='manual'  # Mark as using manual dispatch rule
        )

        try:
            db.session.add(inbound_agent)
            db.session.commit()

            flash(f'Inbound agent "{name}" created successfully using {selected_agent.name}\'s configuration!', 'success')
...
def edit_inbound_agent(agent_id):
    ...
    # Get all outbound agents to select from
    agents = Agent.query.filter_by(
        user_id=session['user_id'],
        call_type='outbound'
    ).order_by(Agent.name).all()

    # Try to find which agent this inbound agent was based on (by matching prompt/greeting/voice)
    current_base_agent = None
    for potential_agent in agents:
        if (potential_agent.prompt == agent.prompt and
            potential_agent.greeting == agent.greeting and
            potential_agent.voice_id == agent.voice_id):
            current_base_agent = potential_agent
            break
    ...
    if request.method == 'POST':
        ...
        if not selected_agent:
            flash('Selected agent not found', 'error')
            return render_template('edit_inbound_agent.html', agent=agent, agents=agents, current_base_agent=current_base_agent)

        # Update with selected agent's configuration
        agent.prompt = selected_agent.prompt
        agent.greeting = selected_agent.greeting
        agent.voice_id = selected_agent.voice_id
        agent.voice_name = selected_agent.voice_name

        try:
            db.session.commit()
            flash(f'Inbound agent "{agent.name}" updated successfully using {selected_agent.name}\'s configuration!', 'success')
            return redirect(url_for('inbound_agents'))
        except Exception as e:
            db.session.rollback()
...
def delete_inbound_agent(agent_id):
    """Delete inbound agent (dispatch rule remains for other agents)"""
    agent = Agent.query.filter_by(
        id=agent_id,
        user_id=session['user_id'],
        call_type='inbound'
    ...
    # Don't delete dispatch rule - it's shared across all agents
    # Just delete the agent from database
    try:
        db.session.delete(agent)
        db.session.commit()
...
def campaigns():
    ...
    for campaign in campaigns:
        ...
        interested = len([c for c in campaign.contacts if c.interest_level == 'Interested'])
        not_interested = len([c for c in campaign.contacts if c.interest_level == 'Not Interested'])
    ...
    return render_template('campaigns.html', campaigns=campaigns, pagination=campaigns_pagination)
...
def create_campaign():
    ...
    return render_template('create_campaign.html', agents=agents)
...
def edit_campaign(campaign_id):
    ...
    agents = Agent.query.filter_by(user_id=session['user_id'], call_type='outbound').all()
    return render_template('edit_campaign.html', campaign=campaign, agents=agents)
...
def view_campaign(campaign_id):
    ...
    return render_template('view_campaign.html', campaign=campaign, stats=stats, recent_contacts=recent_contacts, call_logs_map=call_logs_map, contacts_pagination=contacts_pagination)
...
def call_ended_webhook():
    ...
    try:
        ...
        if is_campaign_call:
            ...
            try:
                ...
                if contact:
                    ...
                    logger.info(f"📞 SIP Status: {sip_call_status}, Messages: {len(messages)}, Duration: {duration}s")

                    if len(messages) > 0:
                        contact.status = 'completed'
                        logger.info(f"✅ Campaign call completed ({len(messages)} messages)")
                        ...
                        contact.interest_level = 'Not Interested'
                    ...
                    if campaign:
                        call_log = CallLog(
                            ...
                            call_type='outbound',
                            status='completed' if len(messages) > 0 else 'no_answer',
                            minutes_used=math.ceil(duration / 60) if len(messages) > 0 else 0,
                            created_at=datetime.now(timezone.utc)
                        )

                        # Add sentiment analysis for campaign calls too with agent context
        ...
        if not call_log:
            ...
            # Find the latest inbound agent to associate with this call
            latest_agent = Agent.query.filter_by(call_type='inbound').order_by(Agent.created_at.desc()).first()

            if latest_agent:
                call_log = CallLog(
                    user_id=latest_agent.user_id,
                ...
                logger.info(f"✅ Created inbound call log (ID: {call_log.id}) for agent: {latest_agent.name}")
            else:
                logger.error("❌ No inbound agent found to create call log")
                return jsonify({'error': 'No inbound agent found'}), 404
        ...
        if call_log:
            ...
            # Determine call status - prioritize messages over SIP status
            messages = metadata.get('messages', [])
            ...
            # Primary rule: If agent spoke (messages exist), call was answered → CHARGE
            if len(messages) > 0:
                # At least one message exists (agent spoke) - call was connected, charge it
                # This is the most reliable indicator that call was answered
            ...
            # Secondary rule: Check SIP status only if no messages
            elif sip_call_status in ['dialing', 'ringing']:
                # Call was never answered (and agent never spoke) - no charge
            ...
            # Perform interest analysis on conversation with agent context
            messages = metadata.get('messages', [])
            if messages and len(messages) > 0:
            ...
            if call_log.status == 'completed' and call_log.minutes_used > 0:
                user = db.session.get(User, call_log.user_id)
                if user:
                    user.minutes_balance -= call_log.minutes_used
                    if user.minutes_balance < 0:
                        user.minutes_balance = 0
                    # Track total minutes used
                    user.minutes_used = (user.minutes_used or 0) + call_log.minutes_used
                    logger.info(f"💰 Deducted {call_log.minutes_used} minutes from user {user.username}")
            else:
                logger.info(f"💸 No minutes deducted (status: {call_log.status})")
