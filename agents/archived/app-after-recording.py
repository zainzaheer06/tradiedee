...
def inbound_trunk_management():
    ...
    return render_template('admin_inbound_trunk_management.html', configs=inbound_configs)
...
def configure_inbound_trunk(config_id):
    ...
    return render_template('admin_configure_inbound_trunk.html', config=config)
...
def inbound_agents():
    ...
    return render_template('inbound_agents.html', configs=configs)
...
def create_inbound_agent():
    ...
    if request.method == 'POST':
        ...
        if not selected_agent:
            ...
            return render_template('create_inbound_agent.html', agents=agents, form_data=request.form)
...
def edit_inbound_agent(config_id):
    ...
    if request.method == 'POST':
        ...
        if not selected_agent:
            flash('Selected agent not found', 'error')
            return render_template('edit_inbound_agent.html', config=config, agents=agents)
    ...
    try:
            ...
            try:
                ...
                if contact:
                    ...
                    if campaign:
                        ...
                        if recording_id and len(messages) > 0:
                            logger.info(f"🎙️ Processing recording {recording_id} for campaign call...")
                            try:
                                ...
                                if recording_url:
                                    ...
                                    logger.warning("⚠️ Failed to process recording")
                            except Exception as e:
        ...
        if call_log:
            ...
            if recording_id and call_log.status == 'completed':
                logger.info(f"🎙️ Processing recording {recording_id} for call...")
                try:
                    # Process recording: download from LiveKit, upload to OSS
                    recording_url = asyncio.run(
                        recording_service.process_recording(recording_id, room_name)
