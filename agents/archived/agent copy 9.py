...
IMPORTANT:
Carefully Understand the user context in term on regional way or direct way: If the user intends to end or close the conversation — for example, by saying phrases like “وداعاً”, “مع السلامة”, “باي”, “شكراً ما أحتاج”, or by clearly indicating they don’t want to continue — you must immediately trigger the end_call function without after good bye.
...
MUST DO: RESPOND IN CLEAN TEXT WITH ONE SENTENCE OR TWO MAX.SINCE IT IS GOING TO BE SPOKEN ALOUD.

Always give short responses under 150 characters. Don't go in details.

Always respond in Arabic ONLY.

MUST DO: Always stay relevant to the user's questions and avoid going off-topic.

Do not do respond with this format:
{"query":"عدد سكان المملكة العربية السعودية 2025"}
...
class Assistant(Agent):
    ...
    async def search_knowledge_base(self, ctx: RunContext, query: str) -> str:
        """
        Search the agent's knowledge base for relevant information to answer the user's question.
        Use this when the user asks about topics that might be in the knowledge base (product info, FAQs, procedures, etc.)

        Args:
            query: The search query or question to look up in the knowledge base

        Returns:
            Relevant information from the knowledge base, or a message if not found
        """
        try:
            ...
            logger.info(f"🔍 Searching knowledge base for agent {agent_id}: {query}")

            # Send a verbal status update to the user if search takes too long
            async def _speak_status_update(delay: float = 0.1):
                await asyncio.sleep(delay)
                '''
                await ctx.session.generate_reply(instructions="""
                    You are searching the knowledge base but it is taking a moment.
                    Update the user on your progress very briefly and short.
                    Example: "دعني أبحث في قاعدة المعلومات..." (Let me search the knowledge base...)
                """)
                '''
                await ctx.session.say("دعني أبحث في قاعدة المعلومات...")

            # Start the status update task (will speak if search takes > 0.1s)
            status_update_task = asyncio.create_task(_speak_status_update(0.1))

            # Query the knowledge base
            result = await kb_service.aquery(agent_id, query, top_k=3)
            ...
            if result:
                logger.info(f"✅ Found knowledge base results: {result[:200]}...")
                return f"Based on the knowledge base: {result}"
            else:
                logger.info("⚠️ No results found in knowledge base")
                return "I couldn't find specific information about that in my knowledge base. Let me try to help based on my general knowledge."

        except Exception as e:
            logger.error(f"❌ Error searching knowledge base: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return "I encountered an error accessing the knowledge base. Let me try to help based on my general knowledge."
