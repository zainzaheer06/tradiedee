...
async def entrypoint(ctx: JobContext):
    ...
    # Check for demo/test rooms that shouldn't be processing
    if ctx.room.name.startswith('demo-'):
        logger.warning(f"⚠️  DEMO ROOM DETECTED: {ctx.room.name} - This may be a test room")
    ...
    tts_engine = elevenlabs.TTS(
        ...
        streaming_latency=0,  # 0 = Maximum speed, 4 = Maximum quality
    ...
    async def send_call_data():
        ...
        try:
            ...
            logger.error(f"❌ Webhook timeout after 20 seconds")
        except Exception as e:
    ...
    # --- NEW: Pre-load tools (uses cache from startup) ---
    dynamic_tools = []
    if agent_id:
        try:
            dynamic_tools = tool_service.create_function_tools(agent_id)
    ...
    await session.start(
        room=ctx.room,
        agent=Assistant(config=agent_config, agent_id=agent_id, preloaded_tools=dynamic_tools),  # ← Pass tools
    ...
    await asyncio.sleep(0.2)  # Give SIP time to establish media connection
